...
namespace Settings
{
	...
	const float analog_vref(2.4976);  //value of voltage reference used for analog inputs
	...
	const float dac_vref(1.2470); //value of DAC's internal voltage reference
        const byte dac_bits(12); //number of bits used by DAC
        const float dac_offset[4] = {0.0005,0.0024,-0.0008,0.0034}; //offset between the commanded and actual DAC voltage, typically ~1-2 mV
	...
	const float int_tc_max(10.0); //max allowed integrator time constant (sec) (above 3.4sec each turn of rot encoder doesn't always change pot value)
        ...
        const unsigned long pot_max[4] = {222300UL, 223300UL, 229500UL, 228400UL};
        const unsigned long pot_min[4] = {1080UL, 1100UL, 1100UL, 1120UL};
        ...
        const byte versionNum(5); //Calibrated 1/11/16 BJR
