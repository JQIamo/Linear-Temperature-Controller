...
namespace Settings
{
	...
	const float analog_vref(2.4991);  //value of voltage reference used for analog inputs
	...
	const float dac_vref(1.2480); //value of DAC's internal voltage reference
        const byte dac_bits(12); //number of bits used by DAC
        const float dac_offset[4] = {-0.0003,-0.0017,0.0014,-0.0057}; //offset between the commanded and actual DAC voltage, typically ~1-2 mV
	...
	const float int_tc_max(10.0); //max allowed integrator time constant (sec) (above 3.4sec each turn of rot encoder doesn't always change pot value)
        ...
        const unsigned long pot_max[4] = {225700UL, 227100UL, 223400UL, 218600UL};
        const unsigned long pot_min[4] = {1120UL, 1130UL, 1100UL, 1080UL};
        ...
        const byte versionNum(3);  //Calibration Date 1/11/16 BJR
