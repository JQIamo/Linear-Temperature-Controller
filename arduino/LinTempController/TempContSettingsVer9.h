...
namespace Settings
{
	...
	const float analog_vref(2.4990);  //value of voltage reference used for analog inputs
	...
	const float dac_vref(1.2469); //value of DAC's internal voltage reference
        const byte dac_bits(12); //number of bits used by DAC
        const float dac_offset[4] = {-0.0043,0.0014,-0.0020,-0.0023}; //offset between the commanded and actual DAC voltage, typically ~1-2 mV
	...
	const float int_tc_max(10.0); //max allowed integrator time constant (sec) (above 3.4sec each turn of rot encoder doesn't always change pot value)
        ...
        const unsigned long pot_max[4] = {223100UL, 227600UL, 224600UL, 231800UL};
        const unsigned long pot_min[4] = {1100UL, 1120UL, 1140UL, 1120UL};
        ...
        const byte versionNum(9); //Calibrated 1/12/16
