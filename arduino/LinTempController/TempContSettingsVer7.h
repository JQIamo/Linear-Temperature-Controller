...
namespace Settings
{
	...
	const float analog_vref(2.4983);  //value of voltage reference used for analog inputs
	...
	const float dac_vref(1.2480); //value of DAC's internal voltage reference
        ...
        const float dac_offset[4] = {0.0004,0.0004,0.0002,-0.0037}; //offset between the commanded and actual DAC voltage, typically ~1-2 mV
	...
	const float int_tc_max(10.0); //max allowed integrator time constant (sec) (above 3.4sec each turn of rot encoder doesn't always change pot value)
        ...
        const unsigned long pot_max[4] = {224300UL, 224200UL, 224500UL, 224600UL};
        const unsigned long pot_min[4] = {1100UL, 1100UL, 1060UL, 1060UL};
        ...
        const byte versionNum(7); //Calibrated 1/12/16 
