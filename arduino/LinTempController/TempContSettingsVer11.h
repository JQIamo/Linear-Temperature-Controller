...
namespace Settings
{
	...
	const float analog_vref(2.4989);  //value of voltage reference used for analog inputs
	...
	const float dac_vref(1.2479); //value of DAC's internal voltage reference
        ...
        const float dac_offset[4] = {0.0013,0.0023,0.0006,-0.0005}; //offset between the commanded and actual DAC voltage, typically ~1-2 mV
	...
	const float int_tc_max(10.0); //max allowed integrator time constant (sec) (above 3.4sec each turn of rot encoder doesn't always change pot value)
        ...
        const unsigned long pot_max[4] = {226300UL, 224900UL, 224900UL, 226200UL};
        const unsigned long pot_min[4] = {1121UL, 1120UL, 1118UL, 1108UL};
        ...
        const byte versionNum(11); //Calibrated 1/12/16
