...
namespace Settings
{
	...
	const float analog_vref(2.4994);  //value of voltage reference used for analog inputs
	...
	const float dac_vref(1.2462); //value of DAC's internal voltage reference
        const byte dac_bits(12); //number of bits used by DAC
        const float dac_offset[4] = {-0.0005,-0.0022,-0.0015,-0.0023}; //offset between the commanded and actual DAC voltage, typically ~1-2 mV
	...
	const float int_tc_max(10.0); //max allowed integrator time constant (sec) (above 3.4sec each turn of rot encoder doesn't always change pot value)
        ...
        const unsigned long pot_max[4] = {222000UL, 229000UL, 224100UL, 224200UL};
        const unsigned long pot_min[4] = {1092UL, 1120UL, 1100UL, 1110UL};
        ...
        const byte versionNum(10); //Calibrated 1/12/16
