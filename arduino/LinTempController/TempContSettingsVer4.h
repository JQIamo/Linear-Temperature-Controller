...
namespace Settings
{
	...
	const float analog_vref(2.4995);  //value of voltage reference used for analog inputs
	...
	const float dac_vref(1.2466); //value of DAC's internal voltage reference
        ...
        const float dac_offset[4] = {-0.0001,-0.0024,0.0001,0.0008}; //offset between the commanded and actual DAC voltage, typically ~1-2 mV
	...
	const float int_tc_max(10.0); //max allowed integrator time constant (sec) (above 3.4sec each turn of rot encoder doesn't always change pot value)
        ...
        const unsigned long pot_max[4] = {228500UL, 218400UL, 228500UL, 225700UL};
        const unsigned long pot_min[4] = {1070UL, 1050UL, 1080UL, 1100UL};
        ...
        const byte versionNum(4);    //Calibration 1/11/16 BJR
